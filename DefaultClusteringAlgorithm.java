...
public class DefaultClusteringAlgorithm implements ClusteringAlgorithm {
  ...
  @Override
  public HCCluster performWeightedClustering(double[][] distances, String[] clusterNames,
    ...
    /* Setup model */
    List<HCCluster> clusters = createClusters(clusterNames, weights);
    DistanceMap linkages = createLinkages(distances, clusters);
  ...
  private DistanceMap createLinkages(double[][] distances,
	...
	private List<HCCluster> createClusters(String[] clusterNames) {
		List<HCCluster> clusters = new ArrayList<HCCluster>();
        for (String clusterName : clusterNames) {
            HCCluster cluster = new HCCluster(clusterName);
            clusters.add(cluster);
        }
		return clusters;
	}

  private List<HCCluster> createClusters(String[] clusterNames, double[] weights) {
    List<HCCluster> clusters = new ArrayList<HCCluster>();
    for (int i = 0; i < weights.length; i++) {
      HCCluster cluster = new HCCluster(clusterNames[i]);
