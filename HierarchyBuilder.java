...
public class HierarchyBuilder {
    ...
    private List<HCCluster> clusters;

    public DistanceMap getDistances() {
        return distances;
    }

    public List<HCCluster> getClusters() {
        return clusters;
    }

    public HierarchyBuilder(List<HCCluster> clusters, DistanceMap distances) {
    ...
    public void agglomerate(LinkageStrategy linkageStrategy) {
        ...
        if (minDistLink != null) {
            ...
            HCCluster oldClusterL = minDistLink.getlCluster();
            HCCluster oldClusterR = minDistLink.getrCluster();
            HCCluster newCluster = minDistLink.agglomerate(null);

            for (HCCluster iClust : clusters) {
    ...
    private ClusterPair findByClusters(HCCluster c1, HCCluster c2) {
    ...
    public HCCluster getRootCluster() {
        if (!isTreeComplete()) {
